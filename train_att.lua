...
cmd:option('-att_folder', '../data/c3d_att_feat', 'c3d feature for attention')
cmd:option('-vid_file', './data/train_charades.txt', 'filenames of the train-val-test')
...
cmd:option('-att_seq_size', 140, 'seq size of the attention feat map')
...
cmd:option('-learning_rate_decay_start', 1000, 'at what iteration to start decaying learning rate? (-1 = dont)')
...
if string.len(opt.init_from) > 0 then
  ...
  protos.dec = nn.LanguageAtt(lmopt)
  -- create loss funtion 
  protos.crit = nn.LanguageModelCriterion()
end
...
local function eval_split(split)
  ...
  while true do
    ...
    local logprobs = protos.dec:forward({state, data.labels, att_emb, data.att_mask})
    local loss = protos.crit:forward(logprobs, data.labels)
    loss_sum = loss_sum + loss
    loss_evals = loss_evals + 1 

    --print('att_emb size is ', #state)
    local seq = protos.dec:sample{state, att_emb, data.att_mask}
...
local function lossFun()
  ...
  --print init_state for debug
  local logprobs = protos.dec:forward{init_state, data.labels, att_emb, data.att_mask}
  local loss = protos.crit:forward(logprobs, data.labels)
  ...
  local dstate, dimgs = unpack(protos.dec:backward({init_state, data.labels}, dlogprobs)) 
