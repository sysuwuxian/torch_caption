...
function layer:__init(opt)
  ...
  if opt.useLSTM then
    model = LSTM.lstm(self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.num_layers, dropout)
  else
    model = GRU.gru(self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.num_layers, dropout)
  end
  -- +1 for start tokens
...
function layer:sample_beam(input, opt)
  ...
  for k=1,batch_size do
    ...
    for h=1, self.use_lstm and 2 * self.num_layers or self.num_layers do
        state[h] = input[h][k]:repeatTensor(beam_size):view(beam_size, -1)
    end
...
function layer:updateOutput(input)
  ...
  for t=1,self.seq_length+1 do
    ...
    if not can_skip then
      self.inputs[t] = {it, unpack(self.state[t-1])}
...
function layer:updateGradInput(input, gradOutput)
  ...
  self.gradInput = {dstate[0], torch.Tensor()}
